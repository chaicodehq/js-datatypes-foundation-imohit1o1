...
export function repeatPattern(pattern, times) {
  // Your code here
  if (typeof pattern !== "string" || typeof times !== "number" || !Number.isSafeInteger(times) || times <= 0) return "";
  return pattern.repeat(times);
}

export function extractRangoliCenter(design, start, end) {
  // Your code here
  if (typeof design !== "string" || typeof start !== "number" || typeof end !== "number" || start.length === 0 || end.length === 0) return "";
